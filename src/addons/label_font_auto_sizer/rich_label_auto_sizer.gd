...
@export var _current_font_size: int
@export var _last_size_state: LABEL_SIZE_STATE = LABEL_SIZE_STATE.IDLE
@export var _size_just_modified_by_autosizer: bool = false
@export var _editor_defaults_set: bool = false
...
func _set(property: StringName, value: Variant) -> bool:
	match property:
		"text":
			text = value
			_update_texture_caches(value)
			call_deferred(_check_line_count.get_method())
			return true
		_:
			return false

func _update_texture_caches(new_text: String) -> void:
...
func set_bbcode(bb_code: String) -> void:
	# wrapper for parse_bbcode
	# use this instead of parse_bbcode in order to trigger automatic resizing
	_update_texture_caches(bb_code)
	parse_bbcode(bb_code)
	call_deferred(_check_line_count.get_method())
